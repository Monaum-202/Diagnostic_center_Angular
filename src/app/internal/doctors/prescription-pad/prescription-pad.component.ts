...
export class PrescriptionPadComponent implements OnInit {
  ...
  filteredInvestigations: any[][] = []; // Store filtered investigations for each row
    ...
    ngOnInit(): void {
      ...
      this.investigationsArray = this.prescriptionForm.get('investigations') as FormArray;
      ...
      this.http.get<any[]>('http://localhost:9090/api/diagnostic').subscribe(data => {
    ...
    filterInvestigations(index: number) {
      ...
      this.filteredInvestigations[index] = this.allInvestigations.filter(inv =>
